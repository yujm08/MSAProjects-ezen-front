const API_BASE_URL = `${process.env.NEXT_PUBLIC_API_BASE_URL}/data-collector-service/api/global-stock`;

/**
 * ✅ 특정 종목의 주식 데이터를 가져오는 함수 (기간별 데이터 조회)
 * @param stockCode - 종목 코드 (예: "AAPL", "TSLA")
 * @param period - 조회할 기간 ("today", "yesterday", "oneweek", "onemonth", "threemonth")
 * @returns Chart.js 형식의 데이터
 */
export async function fetchGlobalStockData(stockCode: string, period: string) {
    try {
        const response = await fetch(`${API_BASE_URL}/${period}?stockCode=${stockCode}`);

        if (response.status === 404) {
            console.log(`${stockCode} ${period} 데이터가 없습니다.`);
            return null;
        }
        ...
        console.error("fetchGlobalStockData Error:", error);
        throw error;
    }
}
 ...
 */
export async function fetchGlobalStockSummary(stockCode: string) {
    try {
        const response = await fetch(`${API_BASE_URL}/summary?stockCode=${stockCode}`);

        if (response.status === 404) {
            console.log(`${stockCode} 요약 데이터가 없습니다.`);
            return null;
        }
        ...
        console.error("fetchGlobalStockSummary Error:", error);
        throw error;
    }
}
